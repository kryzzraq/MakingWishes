    ...
    >
      ...
      >
        ...
        <template v-slot:action="{ attrs }">
          <v-btn color="white" text v-bind="attrs" @click="chngPss = false">
            Ok
          </v-btn>
        </template>
      </v-snackbar>
      ...
      >
        ...
        <template v-slot:action="{ attrs }">
          <v-btn color="white" text v-bind="attrs" @click="fatalData = false">
            Ok
          </v-btn>
        </template>
      </v-snackbar>
      <v-dialog v-model="changePass" persistent max-width="600px">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="secondary" text @click="changePass = false">
              Cerrar
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-card class="mx-auto pb-10" min-width="325" width="80%">
        <div>
          <v-img
      ...
      if (this.$v.$invalid) {
        ...
        if (this.password.password === this.password.confirm) {
          ...
          let response = await axios.post(
            process.env.VUE_APP_SERVER_TOTAL_PATH + "/changePassAdmin",
            {
              id_user: this.$router.history.current.params.id,
              pass1: newPass1,
              pass2: newPass2,
