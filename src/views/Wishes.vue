<template>
  <v-container class="py-12 py-md-16" :key="forceRender">
    ...
    >
      ...
      <template v-slot:action="{ attrs }">
        <v-btn color="white" text v-bind="attrs" @click="savedWish = false">
          Ok
        </v-btn>
      </template>
    </v-snackbar>
    <div
      v-if="Object.keys(this.wishes).length === 0"
      class="mt-10 d-flex flex-column align-center px-4"
    >
    ...
    <div v-else class="mt-10 d-flex flex-column align-center px-4">
      <h3 class="text-center my-3">
        En salmón aparecerán los deseos que aún tienes disponibles
      </h3>
      <div class="d-flex flex-wrap justify-center">
        <div v-for="item in wishes" v-bind:key="item.id_item" class="">
          ...
          >
            ...
            <v-card-subtitle class="text-lowercase caption py-0 pt-4"
              ><a class="text-white">{{ item.link }}</a></v-card-subtitle
            >
      ...
      <h3 class="text-center my-3 mt-16">
        Sin embargo, en gris aparecerán los que ya han sido seleccionados
      </h3>
      <div class="d-flex flex-wrap justify-center">
        <div v-for="item in wishes" v-bind:key="item.id_item">
          ...
          >
            ...
            <v-card-subtitle class="text-lowercase caption py-0"
              ><a>{{ item.link }}</a></v-card-subtitle
            >
    ...
    <v-container
      min-width="300px"
      class="pb-8 mt-10 d-flex flex-column align-center px-4"
    >
      <div class="pa-4">
        Aquí podrás añadir todos los deseos que quieras que los demás conozcan.
      </div>
      <v-expansion-panels hover>
        <v-expansion-panel>
          <v-expansion-panel-header ripple
            >Despliega si quieres saber más acerca de las listas de
            deseos:</v-expansion-panel-header
          >
          <v-expansion-panel-content>
            <div>
              Las listas de deseos se conforman de los siguientes elementos:
            </div>
      ...
      >
        <v-card-text>
          ...
          <div class="err">
            <div
              v-if="
                $v.new_wish.wish_name.$dirty && !$v.new_wish.wish_name.required
              "
            >
          ...
          <div class="err">
            <div
              v-if="
                $v.new_wish.wish_link.$dirty && !$v.new_wish.wish_link.required
              "
            >
              Este campo es requerido.
            </div>
            <div
              v-if="
                $v.new_wish.wish_link.$dirty && !$v.new_wish.wish_link.maxLength
              "
            >
        ...
        <v-card-text class="d-flex justify-end">
          <v-btn @click="cleanData" text color="white" class="mr-2"
            >Limpiar</v-btn
          >
...
export default {
    ...
    return {
      description: [
        ...
        },
        {
          ...
          example:
            "https://www.zara.com/es/es/camisa-solapa-bordada-p03564096.html?v1=108980157&v2=1718175",
        },
        {
          name: "Disponibilidad:",
          description:
            "Aquí se muestra si alguien ha seleccionado ya tu deseo o no.",
          example: "",
        },
      ],
      new_wish: {
  ...
  methods: {
    ...
    async createNewWish() {
      if (!this.$v.$invalid) {
        let response = await axios.post(
          process.env.VUE_APP_SERVER_TOTAL_PATH + "/createWish",
          {
        ...
        if (response.data.text) {
          this.savedWish = true;
          let responseWishes = await axios.get(
            process.env.VUE_APP_SERVER_TOTAL_PATH + "/loadUserWishes"
          );
    ...
    async deleteWish(id) {
      let response = await axios.post(
        process.env.VUE_APP_SERVER_TOTAL_PATH + "/deleteWish",
        {
          id_item: id,
        }
      );
