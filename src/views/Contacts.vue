  ...
  <v-container class="py-12 py-md-16" :key="forceRender">
    ...
    >
      ...
      <template v-slot:action="{ attrs }">
        <v-btn color="white" text v-bind="attrs" @click="dialogDelete = false">
          Ok
        </v-btn>
      </template>
    </v-snackbar>
    ...
    <div
      v-if="Object.keys(this.contacts).length === 0"
    ...
    <div class="pt-12">
      <v-data-table
        ...
        class="elevation-1"
      >
        <template v-slot:item.route_image="{ item }">
          <v-avatar tile>
            <img
        ...
        <template v-slot:item.name="{ item }">
          <span class="name"
            >{{ item.name }} {{ item.last_name_1 }} {{ item.last_name_2 }}</span
          >
        </template>
        <template v-slot:top>
          <v-toolbar-title class="px-4 py-8">
            <h4 class="white--text pb-3">Mis contactos</h4>
            <v-text-field
              dense
              v-model="search"
              color="secondary"
  ...
  methods: {
    ...
    async deleteItemConfirm(item) {
      let response = await axios.post(
        process.env.VUE_APP_SERVER_TOTAL_PATH + "/deleteContact",
        {
