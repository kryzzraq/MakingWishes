<template>
  <div class="app" :key="force_render">
        ...
        <v-container style="width: 100%" fluid class="d-flex align-center">
          <div v-for="item in menu" v-bind:key="item.index">
            <v-btn
              ...
              ><span class="">{{ item.index }}</span></v-btn
            >
          </div>
          <v-spacer></v-spacer>
          <div>
            ...
            <v-card>
              <v-list-item-content class="justify-center">
                <div class="mx-auto text-center">
                  <v-avatar color="secondary" class="mb-3">
                    <img
                      :src="user.image"
                      alt="profile-avatar"
                      class="profile-avatar"
    ...
    <div class="hidden-lg-and-up">
      <v-card class="pt-1">
        <v-app-bar app color="primary" tile dark>
          <v-app-bar-nav-icon
            @click="drawer = true"
            class="black--text"
          ></v-app-bar-nav-icon>
        ...
        <v-navigation-drawer v-model="drawer" absolute temporary class="pa-2">
          <v-list nav dense flat>
            <v-list-item-title class="title pb-2"
              >Making Wishes
            </v-list-item-title>
            <v-list-item-group v-for="item in menu" v-bind:key="item.index">
              <v-list-item>
                <v-icon size="20px" class="pa-3 grey--text text--darken-3"
            ...
            <v-divider class="my-3"></v-divider>
            <v-list-item-group v-for="item in submenu" :key="item.index">
            ...
            <v-list-item @click="signOff" color="primary" class="mt-4">
              <v-list-item-title class="primary--text text-center text-button"
    ...
    <v-footer dark padless>
      ...
      >
        <v-card-text class="white--text">
          ...
          <div>
            Cuenta a tus amigos lo que siempre has querido que te regalen.
            Escucha lo que siempre quisieron que les regalases.
          </div>
          <div class="mt-2">
            ¡Ahora es la oportunidad de que todos acierten!
          </div>
        </v-card-text>
        ...
        <v-card-text class="white--text">
          <v-row>
            <v-col>
              <div>
                <a href="https://www.facebook.es"
                  ><v-icon size="30px" class="pa-1 text--white"
                    >mdi-facebook</v-icon
                  ></a
                >
            ...
            <v-col>
              <div>
                <a
                  class="white--text"
                  @click="$router.push('/home/legal-warning')"
                >
                  Aviso legal</a
                >
              </div>
...
export default {
  ...
  methods: {
    async signOff() {
      let response = await axios.get(
        process.env.VUE_APP_SERVER_TOTAL_PATH + "/signoff"
      );
    ...
    searchContacts() {
      ...
      this.$router.push("/home/searchcontacts").catch(() => {});

      if (this.$router.history.current.fullPath === "/home/searchcontacts") {
        this.force_render += 1;
      }
    },
    ...mapActions(["actSearchContacts"]),
  },
  computed: {
    ...mapState(["user"]),
  },
  ...
  beforeUpdate() {
    this.user.image =
      process.env.VUE_APP_SERVER_IMG_PATH + this.user.route_image;
  },
};
</script>
<style scooped>
